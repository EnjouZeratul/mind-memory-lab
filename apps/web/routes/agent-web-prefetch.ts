import type { AgentToolCallTrace, WebSearchHit, WebSearchPort } from "@mind/agent-contract";

/**
 * 是否在助手回合做「环境」联网检索（与具体问法无关，一条查询用用户原文截断）。
 * 显式开关：`MIND_AGENT_AMBIENT_PREFETCH=0|false|off` 关闭。
 */
export function ambientWebPrefetchEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
...
export async function runServerWebPrefetch(
  webSearch: WebSearchPort,
  userText: string,
  env: NodeJS.ProcessEnv = process.env,
): Promise<ServerPrefetchResult> {
...
export function mergeAgentToolTrace(
  server: AgentToolCallTrace[],
  model?: AgentToolCallTrace[],
): AgentToolCallTrace[] {
  return [...server, ...(model ?? [])];
}
