...
import { runUnifiedAgentTurnJson, runUnifiedAgentTurnStream, type UnifiedChatDeps } from "./routes/unified-agent-chat.js";

const dataDir = path.resolve(process.env.MIND_DATA_DIR ?? path.join(here, ".data"));
const memoryRoot = path.join(dataDir, "memory");
const workspaceRoot = path.resolve(
  process.env.MIND_WORKSPACE_ROOT ?? path.join(here, "..", ".."),
);

async function logMemoryEvent(kind: string, payload: Record<string, unknown>): Promise<void> {
  if (process.env.MIND_MEMORY_EVENTS !== "1") return;
...
class SessionAdapter implements SessionPort {
...
class RecallAdapter implements RecallPort {
...
class EnvComputeRouting implements ComputeRoutingPort {
  async resolveSurface(_input: TurnRoutingInput): Promise<"local" | "cloud"> {
    return process.env.MIND_COMPUTE_DEFAULT === "cloud" ? "cloud" : "local";
  }
}

class NoopNeuromorphicHints implements NeuromorphicHintPort {
  async spikeBudgetFor(): Promise<{ maxSpikes: number } | null> {
    return null;
  }
}
...
app.post("/api/sessions", async (req, res) => {
  try {
    const raw = String((req.body as { id?: string })?.id ?? "").trim();
...
const unifiedChatDeps: UnifiedChatDeps = {
  session: new SessionAdapter(),
  recall: new RecallAdapter(),
  workspace: wsReader,
  reader: wsReader,
...
app.get("/api/memory/entries", async (req, res) => {
  try {
    ...
    const layer = (["working", "longterm", "archive"] as const).includes(lr as MemoryLayer)
      ? (lr as MemoryLayer)
      : undefined;
    const status = (["draft", "stable", "archived"] as const).includes(sr as MemoryStatus)
      ? (sr as MemoryStatus)
      : undefined;
    const page = await listMemoryCatalogPage(
      memoryRoot,
      { categoryPrefix: category || undefined, layer, status },
      limit,
      cursor,
    );
    res.json(page);
...
app.post("/api/memory/archive", async (req, res) => {
  try {
    const b = req.body as { path?: string; move?: boolean };
    if (!b.path) {
      res.status(400).json({ error: "path 为必填字段。" });
      return;
    }
...
async function listMarkdownRecursive(
  ...
  for (const e of ents) {
    ...
    if (e.isDirectory()) out.push(...(await listMarkdownRecursive(abs, childRel)));
